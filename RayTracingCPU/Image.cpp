...
namespace Alice::RayTracingCPU::Privete
{
...
namespace Alice::RayTracingCPU
{
	...
	{
		using namespace Alice::RayTracingCPU::Privete;

		std::cout << "Alice is drawing heart on "<< path << " for you." << std::endl;
		...
		for(uint32_t sy = 0; sy < height; sy++)
		{
			...
			PrintRenderProgress(float(sy) / float(height));
		}
